...
describe("ResendEmailProvider", () => {
  ...
  it("envia o e-mail de verificação com um link para /verificar-email", async () => {
    ...
    const provider = new ResendEmailProvider("api-key", "no-reply@togu.app", "https://togu.app");
    ...
    expect(body.from).toBe("no-reply@togu.app");
    expect(body.to).toBe("ana@example.com");
    expect(body.html).toContain("https://togu.app/verificar-email?token=tok123");
  });
  ...
  it("envia o e-mail de redefinição de senha com um link para /redefinir-senha", async () => {
    ...
    const [, options] = call;
    const body = JSON.parse(options.body);
    expect(body.html).toContain("https://togu.app/redefinir-senha?token=tok456");
  });
  ...
  it("lança erro quando a API do Resend responde com falha", async () => {
    ...
    const provider = new ResendEmailProvider("api-key-invalida", "no-reply@togu.app", "https://togu.app");
